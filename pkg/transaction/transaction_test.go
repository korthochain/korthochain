...
func TestCodec(t *testing.T) {
	assert := assert.New(t)
	var max = ^uint64(0)
	fromPriv, _ := sigs.Generate(crypto.TypeED25519)
	fromPub, _ := sigs.ToPublic(crypto.TypeED25519, fromPriv)
	...
	siganature, err := sigs.Sign(crypto.TypeED25519, fromPriv, tx.SignHash())
